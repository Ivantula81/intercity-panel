...
## Конвенции
- Перед любой задачей и релизом обязательно прочитать `AGENTS.md` и
  `docs/critical-risks-and-change-checklist.md`. Для каждой правки указать затронутые
  `RISK-*`, проверить, не усиливает ли она открытые проблемы, и перечислить
  фактически выполненные проверки.
- Любая правка кода → `./scripts/preflight.sh` → профильные проверки затронутых
  `RISK-*` → PR в `main`. Деплой `/root/deploy.sh` — только после сверки версии
  прода, прохождения стоп-условий и явного решения владельца выпускать релиз.
