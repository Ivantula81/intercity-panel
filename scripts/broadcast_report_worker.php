...
foreach ($groups as $group) { $j=$group['anchor']; $jobIds=$group['ids'];
    ...
    $delivered=(int)($counts['delivered']??0)+(int)($counts['read']??0); $failed=(int)($counts['failed']??0)+(int)($counts['skipped']??0);
    $noMaxSt=$pdo->prepare("SELECT COUNT(*) FROM broadcast_deliveries WHERE job_id IN ($ph) AND (LOWER(last_error) LIKE '%noaccount%' OR LOWER(last_error) LIKE '%нет max%')"); $noMaxSt->execute($jobIds); $noMax=(int)$noMaxSt->fetchColumn();
    $passSt=$pdo->prepare('SELECT COUNT(*) FROM passengers WHERE manifest_id=?'); $passSt->execute([(int)$j['manifest_id']]); $passengers=(int)$passSt->fetchColumn();
    $inProgress=$queued+$accepted; $sentToProvider=$delivered+$accepted;
    ...
    if ($kind==='') continue;
    $del=[]; $body = ($kind==='final' ? 'Окончательный результат рассылки' : 'Промежуточный результат — рассылка продолжается') . "\nВедомость №".$j['trip_number']."\nДата рейса: ".date('d.m.Y H:i',strtotime($j['departure_at']))."\nМаршрут: ".$j['route']."\n\nПассажиров в ведомости: ".$passengers."\nСообщений запланировано: ".$total."\nОтправлено/принято Green API: ".$sentToProvider."\nВ очереди или ожидают статуса: ".$inProgress."\nДоставлено/прочитано: ".$delivered."\nНет MAX: ".$noMax."\nОшибки: ".$failed;
    if ($kind==='progress') $body .= "\n\nЭто промежуточный отчёт. Рассылка продолжается, окончательный результат будет после завершения.";
    if ($kind==='final' && $accepted > 0) $body .= "\n\nЧасть сообщений всё ещё без финального статуса Green API — данные могут уточняться.";
